...
const contractsDrawer = observer(({type,selType,states,data,othersVal,init}) => {
    ...
    useEffect(() => {       
        ...
        if(init.range && init.range != 'others'){
    ...
    const handleChangeInput = useCallback(
        (type) => (e) => {
            ...
            }else if(type ==='deduction_rate' || type ==='payment_rate'){
                stateData[type] = Math.max(0, Math.min(100, e.target.value.replace(/([^0-9])/g, '')));
            }else{
            ...
            {stateData.sel_type !=='I'                     
                ...
                <Wrapper>
                    <Row gutter={10} className="table marginUp">
                        ...
                        <Col xs={19} lg={19}>
                            <DatePicker value={stateData.start_date} name="start_date" disabledDate={e=>handleCheckDate('start_date',e)} onChange={handleChangeInput('start_date')} style={{width: 115, textAlign: 'right'}} />
                            ~<DatePicker value={stateData.end_date}  name="end_date" disabledDate={e=>handleCheckDate('end_date',e)} onChange={handleChangeInput('end_date')} style={{width: 115, textAlign: 'right'}}/> 이며, &nbsp;
