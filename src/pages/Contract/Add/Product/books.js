...
const contractsDrawer = observer(({type,selType,states,data,booksVal,init}) => {
    ...
    useEffect(() => {
        ...
        if(init.range && init.range != 'book'){
            if(init.start_date)stateData.start_date = init.start_date;
            if(init.end_date)stateData.end_date = init.end_date;
            if(init.extension_year)stateData.extension_year = init.extension_year;
            if(init.brokers){
                stateData.brokers = toJS(init.brokers);
    ...
    const handleChangeInput = useCallback(
        (type) => (e) => {
            ...
            }else if(type === 'prepaid_royalty' || type === 'exemption_royalty_qty' || type==='data_cost'){
            ...
            {state.states ==='insert'                     
                ...
                <Row gutter={10} className="table marginUp">
                    ...
                    <Col xs={19} lg={19}>
                        ...
                        <Input type="number" min="0" max="10" name="extension_year" value={stateData.extension_year} onChange={handleChangeInput('extension_year')} autoComplete="off" style={{width: 55, textAlign: 'right'}}/>년 단위로 자동 연장
                        (자동 연장 없으면 0으로 입력)
                    </Col>
                    <Col xs={5} lg={5} className="label">
                    ...
                    <Col xs={19} lg={19}>
                        ...
                        <DatePicker value={stateData.prepaid_royalty_date} disabledDate={e=>handleCheckDate('prepaid_royalty_date',e)} name="prepaid_royalty_date" onChange={handleChangeInput('prepaid_royalty_date')} style={{width: 115, textAlign: 'right'}} />까지 지급
                    </Col>
                    <Col xs={5} lg={5} className="label">
                            ...
                            <div style={index > 0 ? {marginTop: 10} : {}}>
                                <Space>
                                    ...
                                    <span style={{display: 'inline-block', width: 70, textAlign: 'right'}}>
                                        {index==0 ? 1 : (stateData.fixed_royalties[index-1].qty !== '' ? (parseInt(stateData.fixed_royalties[index-1].qty)+1) : 'n')}
                                        부 ~ 
                                    </span>
                                    <Input type="text" name="qty" value={item.qty.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} onChange={handleChangeRoyalty(index,'qty')} onBlur={handleBlurRoyalty(index)} style={{width: 70, textAlign: 'right'}}/>부 또는 
                                    <Checkbox defaultChecked={false} name="end_yn" onChange={handleChangeRoyalty(index,'end_yn')}/> 끝까지 : 
                                    <Input type="number" min="0" name="percent" value={item.percent} onChange={handleChangeRoyalty(index,'percent')} style={{width: 65, textAlign: 'right'}}/> %
                                    ...
                                    //     style={{ marginLeft: '5px' }}
                                    // />
                                    <Button className="btn btn-primary btn_add" shape="circle" onClick={handleInputRoyalty()}>+</Button>
                                    }
                                </Space>
                            </div>
                        ))}
                    ...
                    <Col xs={19} lg={19}>
                        <Input 
                            type="text" 
                            name="exemption_royalty_qty" 
                            value={stateData.exemption_royalty_qty.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} 
                            onChange={handleChangeInput('exemption_royalty_qty')} 
                            autoComplete="off" 
                    ...
                    <Col xs={5} lg={5} className="label">
                        정기 저작권료 정산 주기 <span className="spanStar">*</span>
                    </Col>
                    <Col xs={19} lg={19}>
                        <Radio.Group
                            value={1}
                            // onChange={handleChangeInput('royalty_term_type')}
                        >
                            <Radio value={1}>본사 기준에 따름</Radio>
                            <Radio value={2}>월</Radio>
                            <Radio value={3}>반기</Radio>
                            <Radio value={4}>1년 (현재 본사 기준인 ‘분기’ 외 주기는 계약서에 명시된 경우에만 선택)</Radio>
                        </Radio.Group>
                    </Col>
                    ...
                    <Col xs={19} lg={19}>
                        ...
                        {/* <Button
                            ...
                            onClick={(e) => {
                                showSearchDrawer('brokers');
                            }}
                            size="small"
                            style={{ marginLeft: '5px' }}
                        /> */}
                        <Button className="btn btn-primary btn_add" shape="circle" onClick={(e) => {showSearchDrawer('brokers');}}>+</Button>
                    </Col>
                    }
                </Row>  
                :<></> 
                        ...
                        {stateData.fixed_royalties.map((item,index) => (
                            <div style={index > 0 ? {marginTop: 10} : {}}>
                                <Space>
                                    ...
                                    <Input type="text" name="qty" value={item.qty.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} onChange={handleChangeRoyalty(index,'qty')}  onBlur={handleBlurRoyalty(index)} style={{width: 70, textAlign: 'right'}}/>부 또는 
                                    <Checkbox defaultChecked={false} name="end_yn" onChange={handleChangeRoyalty(index,'end_yn')}/> 끝까지 : 
                                    <Input type="number" min="0" name="percent" value={item.percent} onChange={handleChangeRoyalty(index,'percent')} autoComplete="off" style={{width: 65, textAlign: 'right'}}/> %
                                    {stateData.fixed_royalties.length == index+1 && item.end_yn !== 'Y' &&
                                    // <Button
                                    //     shape="circle"
                                    //     icon={
