...
import ContractFile from './Product/contFile';

import OverseasBasicInfo from './overseasInfo';
...
const ContractInfo = observer(({ type,contractDrawerVisible,contractDrawerClose,getCopyrights,dataInfo,data,selType,getowners } ) => {
                        ...
                        {state.type === 'contracts' ? (
                            <>
                                <TabPane tab="기본 정보" key="basicInfo">
                                    <BasicInfo type={state.type} selType={state.selType} states={'insert'} dataInfo={dataInfo} data={stateData} basicVal={getData}/>
                                </TabPane>
                                
                                <TabPane tab="종이책" key="paperBook" disabled={stateData.copyright_fee_lump_sum!=='N' || !stateData.ranges.includes('book')}>
                                    <PrdBooks type={state.type} selType={selType} states={'insert'} data={toJS(stateData.books)} booksVal={getData} init={state.set_init} /> 
                                </TabPane>

                                <TabPane tab="전자책" key="eBook" disabled={stateData.copyright_fee_lump_sum!=='N' || !stateData.ranges.includes('ebook')}>
                                    <PrdEbooks type={state.type} selType={selType} states={'insert'} data={toJS(stateData.ebooks)} ebooksVal={getData} init={state.set_init}/> 
                                </TabPane>

                                <TabPane tab="오다오북" key="audio" disabled={stateData.copyright_fee_lump_sum!=='N' || !stateData.ranges.includes('audio')}>
                                    <PrdAudios type={state.type} selType={selType} states={'insert'} data={toJS(stateData.audios)} audiosVal={getData} init={state.set_init}/> 
                                </TabPane>

                                <TabPane tab="2차 저작권" key="other" disabled={stateData.copyright_fee_lump_sum!=='N' || !stateData.ranges.includes('other')}>
                                    <PrdOthers type={state.type} selType={selType} states={'insert'} data={toJS(stateData.others)} othersVal={getData} init={state.set_init}/>
                                </TabPane>

                                <TabPane tab="수출 저작권" key="export" disabled={stateData.copyright_fee_lump_sum!=='N' || !stateData.ranges.includes('export')}>
                                    <PrdExports type={state.type} selType={selType} states={'insert'} data={toJS(stateData.exports)} exportsVal={getData} init={state.set_init}/> 
                                </TabPane>
                            ...
                            <>
                                <TabPane tab="기본 정보" key="basicInfo">
                                    <OverseasBasicInfo type={state.type} selType={state.selType} states={'insert'} dataInfo={dataInfo} data={stateData} basicVal={getData}/>
                                </TabPane>
                                ...
                                <TabPane tab="오다오북" key="audio" disabled={!stateData.ranges.includes('audio')}>
                                    <PrdAudios type={state.type} selType={selType} states={'insert'} data={toJS(stateData.audios)} audiosVal={getData} init={state.set_init}/> 
                                </TabPane>
