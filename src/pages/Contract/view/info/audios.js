...
const contractsDrawer = observer(({type,selType,states,data,audiosVal,drawerClass,drawerChk}) => {
    ...
    useEffect(() => {       
        ...
        if(data !== null && data !== undefined){
            if(Object.keys(data).length > 0){
                var temp_type = Object.keys(type==='contracts'?DEF_STATE:(type==='contracts-check'?DEF_STATE:DEF_STATE_OS));
        ...
        // if(stateData.broker_id!==null){
        //     getBrokers(stateData.broker_id);
        // }
    ...
    const getBrokers  = useCallback(async (id) => {
        ...
        .then(function (response) {
            // state.brokers= {id:response.data.data.id ,name : response.data.data.name};
        ...
        <Wrapper>
            ...
            {state.states ==='insert'                     
                ...
                <Row gutter={10} className="table marginUp">
                    ...
                    <Col xs={19} lg={19}>
                        영업 및 판매관리 비용으로 매출의 &nbsp;
                        <Input type="number" min="0" name="fixed_royalty1" value={stateData.fixed_royalty1} onChange={handleChangeInput('fixed_royalty1')} autoComplete="off" style={{width: 55, textAlign: 'right'}}/>%를 공제하고 
                        나머지 금액의 &nbsp;
