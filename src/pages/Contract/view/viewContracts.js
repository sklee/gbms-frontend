...
import { Button, Row, Col,  Modal, Select, Typography } from 'antd';
...
import ContractContents from './contContents';
import AddHolder from '../Search';
import * as ValidationCheck from '../Validator/Adds.js';

import CopyrightGrid from '../Add/copyrightGrid';
...
const contractsDrawer = observer(({idx, type, onClose, tabChk, reset, classChk, drawerClass, drawerChk}) => {
    ...
    const fetchData = useCallback(async (idx) => {
        ...
        if (result.data.data) {
            ...
            result.data.data.copyrights.forEach((e) =>{
                ...
                let temp = {id:'',name:'',status:'',book:'',ebook:'',audio:'',other:'',export:''}
                temp.id = e.id
                temp.name = e.name
                const key_list = ['book','ebook','audio','other','export']
                    ...
                    if(e.pivot.ranges.includes(item)){
                        if(e.pivot[item+'s'].length>0){
                            ...
                            if(result.data.data.check_status==='C'){
                                stateData.copyrights['idx_'+e.id].pivot[item+'s_old'] = [...e.pivot[item+'s'].filter(ex=> !(['N','A','R']).includes(ex.contract_end_yn))]
                            }
                            stateData.copyrights['idx_'+e.id].pivot[item+'s'] = {...e.pivot[item+'s'].find(ex=> (['N','A','R']).includes(ex.contract_end_yn))}
                        }else{
                            temp[item] = '등록 필요'
                        }
                    }else{
                ...
                state.copyrights = [...state.copyrights,temp];
            });
        }
    ...
    //담당자
    const memberData = useCallback(async (idx) => {
        const result = await axios.get(
    ...
    const handleSubmit = useCallback(async (e)=> {
        ...
        if(e !=='I'){
            ...
            for (let i = 0; i < data.copyrights.length; i++) {
                res_validate = ValidationCheck.ModValidation(data,i);
    ...
    return (
        <Wrapper>
            ...
            {tabChk==='V' && (
                <Row gutter={[10, 10]} justify="center" style={{ marginTop: 30 }}>
                    <Col>
                        <Button htmlType="button" onClick={visibleClose}>취소</Button>
                    </Col>
                </Row>
            )}
