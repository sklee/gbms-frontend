...
export function CloseValidation (data) {
    var msg = '';
    var states = false;
    ...
    if(data.settlement_copyright_fee_type === ''){
        msg  = '저작권료/비용 정산을 선택하세요.';
        return {states : states, msg : msg};
    }
    if(data.reason_memo === ''){
        msg  = '해지 사유를 입력하세요.';
    ...
    if(data.progress_memo === ''){
        msg  = '진행 경과를 입력하세요.';
        return {states : states, msg : msg};
    }
    if(data.settlement_copyright_fee_type !== '4'){
        if(data.loss_price === ''){
            msg  = '손실 처리할 금액을 입력하세요.';
            return {states : states, msg : msg};
        }
        if(data.detail_memo === ''){
            msg  = '세부 내역을 입력하세요.';
