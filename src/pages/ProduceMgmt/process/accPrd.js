...
const accPrdList = observer(({tab}) => {
    ...
    const getCompany = useCallback(async () =>{
        ...
        state.produce_company = result.data.data.filter(e => e.process ? e.process.includes('부속 제작') : false);
    },[]);
    ...
    const getProcess = useCallback(async () =>{
        ...
        state.produce_process = result.data.data.filter(e => e.produce_process_code_id ? e.produce_process_code_id=='9' : false);
    },[]);
