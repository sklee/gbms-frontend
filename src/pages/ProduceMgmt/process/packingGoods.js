...
import { Pagination, Button, Row, Col, Modal, Select, Checkbox, DatePicker, Input} from 'antd';
...
const packingGoods = observer(({tab}) => {
    ...
    const getProcess = useCallback(async () =>{
        ...
        state.produce_process = result.data.data.filter(e => e.produce_process_code_id ? e.produce_process_code_id=='7' : false);
    },[]);
                ...
                if (item == state.currentEditItem ) {
                    ...
                    switch (col.binding) {
                        case 'memo' :
                            ...
                            break;
                        case 'price':
                            e.cell.innerHTML = '<button class="btnText title btnRed btn_price_modify">' + commaNum(item.price) +' </button></div>';
                            break;
                        case 'buttons':
                            let btn = '<button id="btnCancel" class="btnText">취소</button>';
                            e.cell.innerHTML = btn;
                            e.cell['dataItem'] = item;
                            break;
                    }
        ...
        <Wrapper>
            ...
            <Row className="gridWrap">       
                ...
                >
                    ...
                    <FlexGridColumn binding="memo" header="참고사항" width="*" minWidth={150} />
