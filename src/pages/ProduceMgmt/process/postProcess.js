...
const postProcessList = observer(({tab}) => {
    ...
    const getProcess = useCallback(async () =>{
        ...
        state.produce_process = result.data.data.filter(e => e.produce_process_code_id===null ? false : e.produce_process_code_id=='4');
    },[]);
    ...
    const initFilter = (filter) => {
        filter.filterColumns = ["produce_company", "produce_process", "paper_standard", "produce_process_unit", "price", "apply_date", "memo", "simulation_yn", "use_yn", "buttons"];
    };
    ...
    const commitEdit=(idx)=> {
            ...
            state.flex.columns.forEach((col) => {
                ...
                if (input) {                    
                    ...
                    if(col.binding === 'produce_format'){
                        state.currentEditItem[col.binding] = input.options[input.selectedIndex].text;     
                        state.currentEditItem['produce_format_id'] = value;
                    }
        ...
        var chk = state.list.find(e=>
            ...
            && e.paper_standard_id === state.currentEditItem.paper_standard_id
            && e.simulation_yn === 'Y'
        )
        if(chk){
        ...
        <Wrapper>
            ...
            <Row className="gridWrap">       
                ...
                >
                    ...
                    <FlexGridColumn binding="produce_process" header="공정" width={150} >
                        <FlexGridCellTemplate
                            cellType="Cell"
                            template={(cell) => {
...
export default postProcessList;
