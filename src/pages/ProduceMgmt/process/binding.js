...
import { Button, Row, Col, Modal, Select, DatePicker, Input } from 'antd';
...
const bindingList = observer(({tab}) => {
    ...
    const getCompany = useCallback(async () =>{
        ...
        state.produce_company = result.data.data.filter(e => e.process ? e.process.includes('제본') : false);
    },[]);
    ...
    const getProcess = useCallback(async () =>{
        ...
        state.produce_process = result.data.data.filter(e => e.produce_process_code_id===null ? false : e.produce_process_code_id=='5');
    },[]);
    ...
    const initFilter = (filter) => {
        filter.filterColumns = ["produce_company", "produce_process", "produce_format", "min_price", "produce_process_unit", "price", "apply_date", "memo", "simulation_yn", "use_yn", "buttons"];
    };
    ...
    const initGrid = (grid) => {    
                ...
                if (item == state.currentEditItem && state.currentEditItem.addCode!==undefined ) {
                    ...
                    switch (col.binding) {
                        ...
                        case 'produce_process_unit' :
                            ...
                            break;
                        case 'min_price' :
                            e.cell.innerHTML = '<input type="number" step="0.01" class="ant-input" name ="'+item.id+'"'+'id="'+col.binding+'"'+'value="'+s.getCellData(e.row, e.col, true).replace(/,/g, "")+'"/>';
                            e.cell['dataItem'] = item;
                            break;
                        ...
                        //     break;
                        // case 'price' : 
                        //     e.cell.innerHTML = '<button class="btnLink title btnRed btn_price_ipt">단가 입력</button></div>'
                        //     e.cell['dataItem'] = item;
                        //     break;
                        case 'price' :
                        case 'apply_date' :
                    ...
                    switch (col.binding) {
                        case 'price':
                            ...
                            if(test){
                                priceWrap ='<button class="btnText title btnRed btn_price">' +price +'</button>';
                            }else{
                                priceWrap ='<button class="btnText title btn_price">' +price +'</button>';
                            }
        ...
        grid.addEventListener(grid.hostElement, 'change', (e) => {
            if(e.target.id == "produce_process"){
                ...
                if(input){
                    if(input.value != ""){
                        state.flex.hostElement.querySelector('#produce_process_unit').value = toJS(state.produce_process).find(element=>element.id==input.value).produce_process_unit;
                    }else{
    ...
    const commitEdit=(idx)=> {
        if (state.currentEditItem) {
            ...
            state.flex.columns.map((col) => {
                ...
                if (input) {
                    if(col.binding ==='produce_format' && wjvalue == "" ){
                        msg = "판형을 선택하세요.";
                        validate = false;
                    }
            ...
            state.flex.columns.forEach((col) => {
                ...
                if (input) {                    
                    ...
                    if(col.binding === 'min_price'){ 
                        state.currentEditItem['min_price'] = value;
                    }
                    // if(col.binding === 'produce_process_unit'){ 
                    //     state.currentEditItem['produce_process_unit'] = value;
                    // }
        ...
        var chk = state.list.find(e=>
            e.produce_process_id === state.currentEditItem.produce_process_id
            && e.produce_format_id === state.currentEditItem.produce_format_id
            && e.simulation_yn === 'Y'
        )
        if(chk){
            state.currentEditItem['simulation_yn'] = 'N';
        ...
        <Wrapper>
            ...
            <Row className="gridWrap">       
                ...
                >
                    ...
                    <FlexGridColumn binding="produce_company" header="제작처" width={'*'} minWidth={150}>
                    ...
                    <FlexGridColumn binding="produce_process" header="공정" width={180} >
                        <FlexGridCellTemplate
                            cellType="Cell"
                            template={(cell) => {
                    ...
                    <FlexGridColumn binding="min_price" header="최소 금액" width={100} align="right" />
                    <FlexGridColumn binding="produce_process_unit" header="단가\n기준" width={70} />
                    <FlexGridColumn binding="price" header="단가" width={80} align="right" />
                    <FlexGridColumn binding="apply_date" header="단가 적용일" width={100} />
                    <FlexGridColumn binding="memo" header="참고사항" width={150} align="left" />
                    <FlexGridColumn binding="simulation_yn" header="시뮬레이션\n적용" width={85} align="center" />
                    <FlexGridColumn binding="use_yn" header="사용\n여부" width={85} align="center" />
            ...
            >
                <Row gutter={10} className="table">
                    ...
                    <Col xs={24} lg={16}>
                        <Input type="number" name="price" step={0.01} onChange={handlePriceInput('price')} value={stateData.price} />
                    </Col> 
                </Row>
            </Modal>
