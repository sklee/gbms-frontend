...
import { Button, Row, Col, Modal, Select, Checkbox, DatePicker, Input, Space } from 'antd';
...
const printingList = observer(({tab}) => {
    ...
    const getCompany = useCallback(async () =>{
        ...
        state.produce_company = result.data.data.filter(e => e.process ? e.process.includes('후가공') : false);
    },[]);
                    ...
                    switch (col.binding) {
                        case 'price':
                            ...
                            for (let index = 0; index < data.length; index++) {
                                ...
                                if(index === 0){
                                    inhtml +='<div>1 ~ ' + obj.qty + ' : <button class="btnText title '+red_chk+' btn_price">' + obj.price +' 원</button></div>';
                                }else if(index == data.length-1){
                                    inhtml +='<div>' + (Number(data[index-1].qty)+1) + ' ~ : <button class="btnText title '+red_chk+' btn_price">' + obj.price +' 원</button></div>';
                                }else{
                                    inhtml +='<div>' + (Number(data[index-1].qty)+1) + ' ~ ' + obj.qty + ' : <button class="btnText title '+red_chk+' btn_price">' + obj.price +' 원</button></div>';
                                }
                            }
                            e.cell.innerHTML = inhtml;
                            e.cell['dataItem'] = item;
    ...
    const editItem = (item) => {
        ...
        tempData.fixed_royalties = tempData.fixed_royalties.map(e=> {delete e.process_printing_price_main_id; return e}) //기존 index 제거 (추가만 함)
    ...
    const commitEdit=(idx)=> {
        if (state.currentEditItem) {
            ...
            var sim_chk = state.list.find(e=>e.print_unit === state.currentEditItem.print_unit && e.paper_standard_id === state.currentEditItem.paper_standard_id
                &&(e.cmyk1 === state.currentEditItem.cmyk1|| e.cmyk2 === state.currentEditItem.cmyk2 || e.cmyk3 === state.currentEditItem.cmyk3 || e.cmyk4 === state.currentEditItem.cmyk4))
            if(sim_chk){
                state.currentEditItem['simulation_yn'] = 'N';
            }
            ...
            if(state.currentEditItem.addCode !== undefined){ //수정일 때는 동작 안함
                if(state.flex.hostElement.querySelector('#print_unit').querySelector('input').value === "본문" ){
        ...
        <Wrapper>
            ...
            <Row className="gridWrap">       
                ...
                >
                    ...
                    <FlexGridColumn binding="price" header="부수 범위와 단가" width={160} align="left" />
                    <FlexGridColumn binding="apply_date" header="단가 적용일" width={100} />
                    <FlexGridColumn binding="memo" header="참고사항" width={100} />
