...
const bindingAddSimList = observer(({tab}) => {
    const { commonStore } = useStore();
    const { Option } = Select;
    const stateData = useLocalStore(() => ({produce_process_id :"", apply_date :"", price :""}));
    const { confirm } = Modal;

    const state = useLocalStore(() => ({
        type : 'simulation-binding-attached',
        list: [],
        selPrice:'',
        historyList:[],
        data: [],
    ...
    const initGrid = (grid) => {    
        ...
        grid.formatItem.addHandler(function (s, e) {
            ...
            if (e.panel == s.cells) {
                ...
                switch (col.binding) {
                    case 'apply_price':
                        e.cell.innerHTML = '<button class="btnText title btnRed btn_price">' + item.apply_price +'</button>';
    ...
    const modalOpen = (obj,item) => {
        ...
        if(obj == 'modify'){
            ...
            stateData.price = item.apply_price;
            ...
            >
                <Row gutter={10} className="table">
                    ...
                    <Col xs={24} lg={16} className="label">
                        <Input type="number" min="0" value={stateData.price} onChange={handlePriceInput('price')} autoComplete="off"  />
                    </Col>
                </Row>
            </Modal>
...
export default bindingAddSimList;
