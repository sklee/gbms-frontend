...
const PRINT_UNIT = ["표지", "본문", "면지", "없음"];

const produceDetailList = observer(({tab}) => {
    ...
    const initFilter = (filter) => {
        filter.filterColumns = [ "name", "print_unit", "use_yn", "buttons" ];
    };
            ...
            if (e.panel == s.cells) {
                ...
                if (item == state.currentEditItem ) {
                    ...
                    switch (col.binding) {
                        ...
                        //             opt += '<option selected>'+ PRINT_UNIT[index] +'</option>';
                        //         }else{
                        //             opt += '<option>'+ PRINT_UNIT[index] +'</option>';
                        //         }
                        //     }
                    ...
                    switch (col.binding) {
                        case 'use_yn':
                            if(e.cell.innerHTML == "Y" ){
                                var checked = 'checked="checked"';
                            }else{
                                var checked = '';
                            }
                            e.cell.innerHTML = '<input id="' + col.binding + item.id + '" type="checkbox" name= "'+col.binding+item.id+'" value="'+e.cell.innerHTML+'" '+checked+' onClick="return false;" />';
                            break;
                        case 'buttons':
    ...
    const rowAdd=(e)=>{    
        if(e === true){ //행추가일때
            ...
            state.currentEditItem = { id: state.addCnt, num: '', print_unit : '',use_yn: '',addCode : '', rowAdd: true};
    ...
    const commitEdit=()=> {
        if (state.currentEditItem) {
            ...
            state.flex.columns.map((col) => {
                ...
                if (input) {
                    if(col.binding==='print_unit' && wjvalue ==""){
                        msg = "인쇄 단가를 선택하세요.";
                        validate = false;
                    }
            ...
            state.data.forEach((e) => {    
                ...
                if(e.id > 0){
                    if(e.id === state.currentEditItem.id){
                        ...
                        e.print_unit= state.currentEditItem.print_unit;
        ...
        <Wrapper>
            ...
            <Row className="gridWrap">       
                ...
                >
                    ...
                    <FlexGridColumn binding="print_unit" header="인쇄단가 적용" width={200}>
...
export default produceDetailList;
