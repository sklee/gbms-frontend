...
import PrdPrePress from './prdDrawer/prepress' // prepress제작처(공정) 검색
...
const Prepress = () => {
    ...
    const onSubmit = (submitData) => {
        ...
        if ( mode === 'create' ) {
            ...
            if (Array.isArray(formikHook.values?.defaults[providerData.defaultRowIndex]?.details[providerData.detailRowIndex].prepresses)) {
                // 있으면 Index 추가
                targetIndex = formikHook.values?.defaults[providerData.defaultRowIndex]?.details[providerData.detailRowIndex].prepresses.length
            }
            else {
                // 초기 값이 없으면
                targetIndex = 0
            ...
            // dataIndex 입력
            formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.dataIndex`,               providerData.dataIndexs?.prepress?.current)
        }
        // 수정이면
        else {
        ...
        // 제작처
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.produce_company`,             submitData.produce_company)
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.produce_company_id`,          submitData.produce_company_id)
        // 납품처
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.delivery_produce_company`,    submitData.delivery_produce_company)
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.delivery_produce_company_id`, submitData.delivery_produce_company_id)
        // 공정
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.produce_process`,             submitData.produce_process)
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.produce_process_id`,          submitData.produce_process_id)
        // 제작 수량
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.qty`,                         submitData.qty)
        // 단가
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.price`,                       submitData.price)
        // 공급가
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.supply_price`,                submitData.supply_price)
        // 부가세
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.vat`,                         submitData.vat)
        //부가세 체크 (Y, N)
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.vat_yn`,                      submitData.vat_yn)
        // 합계
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.total_amount`,                submitData.total_amount)
        // 전달 사항
        formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses.${targetIndex}.memo`,                        submitData.memo)
        // 비용 귀속일
    ...
    const onDelete = () => {
        Modal.confirm({
            ...
            onOk : () => {
                ...
                formikHook.setFieldValue(`defaults.${providerData.defaultRowIndex}.details.${providerData.detailRowIndex}.prepresses`, formikHook.values.defaults[providerData.defaultRowIndex]?.details[providerData.detailRowIndex].prepresses.filter((row, index) => index !== selectedRowIndex))
            }
        })
    }
    // formik Hook 선언
    ...
    const setPrepressDrawerData = (rowData) => {
        // 제작처
    ...
    React.useEffect(() => {
        setSelectedRowIndex(formikHook.values?.defaults[providerData.defaultRowIndex]?.details[providerData.detailRowIndex]?.prepresses.indexOf(selectedRow))
    }, [selectedRow])
    // 수정/추가/취소 등 버튼 클릭 후 액션
    React.useEffect(() => {
        if (mode === 'view') {
            // 수정완료, 취소, 
            formikDetailHook.setValues(formikHook.values.defaults[providerData.defaultRowIndex]?.details[providerData.detailRowIndex].prepresses[selectedRowIndex])
        }
        else if (mode === 'modify') {
            // target을 해당 메뉴로 잡는다
            providerData.setActiveMenuTarget('prepress')
        }
        else if (mode === 'create') {
            // 선택된 값 초기화
            setSelectedRowKeys([])
    ...
    }, [formikHook.values.defaults[providerData.defaultRowIndex]?.details[providerData.detailRowIndex]?.prepresses[selectedRowIndex], mode])
    // 다른 메뉴에서 수정/추가 버튼을 클릭했을 때 초기화
    React.useEffect(() => {
        if (providerData.activeMenuTarget !== 'prepress') {
            setMode('view')
        }
    }, [providerData.activeMenuTarget])
    // 선택한 Row가 달라지면 오른쪽 내용 재구성
    React.useEffect(() => {
        setSelectedRow(formikHook.values.defaults[providerData.defaultRowIndex]?.details[providerData.detailRowIndex].prepresses.filter(row => row.dataIndex == selectedRowKeys[0])[0] ) 
    }, [selectedRowKeys])
        ...
        <Row>
            <FormikProvider value={formikDetailHook}>
                ...
                <Col xs={8} lg={8}>
                    <Table 
                        rowKey      = { 'dataIndex' }
                        dataSource  = { providerData.detailRowIndex !== -1 ? formikHook.values?.defaults[providerData.defaultRowIndex]?.details[providerData.detailRowIndex]?.prepresses : [] }
                        columns     = { columns }
                        size        = { 'middle' }
                        bordered    = { true }
                    ...
                    {(selectedRowIndex !== undefined && selectedRowIndex !== -1) || mode === 'create' ? 
                        <>
                            ...
                            <Row style={{border: 0, margin: '10px 0'}}>
                                ...
                                <Col xs={9} lg={9} className="input_box">{formikDetailHook.values?.produce_process?.name}</Col>
                                <Col xs={3} lg={3} className="label">제작 수량 <span className="spanStar">*</span></Col>
                                <Col xs={9} lg={9} className="input_box"><FormikInput name={`qty`} disabled={mode === 'view' ? true : false}/></Col>
                                ...
                                <Col xs={9} lg={9} className="input_box"><FormikInput name={`supply_price`} disabled={mode === 'view' ? true : false}/></Col>
                                ...
                                <Col xs={9} lg={9} className="input_box"><FormikInput name={`total_amount`} disabled={mode === 'view' ? true : false}/></Col>
                ...
                {/* Drawer Side */}
                {prdDrawer  ? <PrdPrePress viewVisible={prdDrawer}  visibleClose={prdDrawerClose}  setPrepressDrawerData={setPrepressDrawerData}/> : ''}
                {vendDrawer ? <Vendor      viewVisible={vendDrawer} visibleClose={vendDrawerClose} setVendorDrawerData={setVendorDrawerData} drawerVendorTarget={'prepress'} produceTargetId={formikDetailHook.values?.produce_company_id}/> : ''}
            </FormikProvider>
        </Row>
    )
