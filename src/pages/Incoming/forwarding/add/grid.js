...
import SlipCodeDrawer from '@pages/shipping/sale/view';
// const { Option } = Select;

const index = observer(( { drawerClass, drawerChk } ) =>{
                ...
                if(item == state.currentEditItem){
                    ...
                    switch (col.binding) {
                        case 'prdName':
                            ...
                            break;
                        case 'slipCode':
                            e.cell.innerHTML = '<button id="btnDrawer" class="btnLink">'+item.slipCode+'</button>';
                            e.cell['dataItem'] = item;
                            break;
                        case 'work':
    ...
    const [slipCodeDrawer, setSlipCodeDrawer] = useState(false);
    const drawerOpen = () => {
        setSlipCodeDrawer(true)
        if(drawerChk === 'Y'){
            drawerClass('drawerback');
        }
    };
    const drawerClose = () => {
        setSlipCodeDrawer(false)
        drawerClass();
    };
    ...
    return(
        <>
            ...
            {slipCodeDrawer && <SlipCodeDrawer drawerVisible={slipCodeDrawer} drawerClose={drawerClose} />}
        </>
    );
});
