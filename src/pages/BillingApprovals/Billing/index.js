...
import BillingView from './View/index'
...
const billingList = observer((props) => {
    ...
    const fetchData = useCallback(async (val) => {
        ...
        var config = {
            method: 'GET',
            url: process.env.REACT_APP_API_URL +'/api/v1/billings?display='+state.pageArr.pageCnt +'&page='+page+'&sort_by=date&order=desc',
            headers: {
    ...
    const viewData = useCallback(async (type,tit) => {
        const result = await axios.get(
            process.env.REACT_APP_API_URL +'/api/v1/billings/'+state.idx,
            {
                headers: {
                    'Accept': 'application/json',
    ...
    const initFilter = (filter) => {
        filter.filterColumns = ["billing_code", "name", "current_unit_name", "sum_total_amount", "accountable.name", "billed_at", "billing_status", "payment_at", "status"];
    };
    ...
    const initGrid = (grid) => {
        ...
        grid.formatItem.addHandler((s, e) => {
            if (e.panel == s.cells) {
                ...
                switch (col.binding) {
                    case 'name':
                        // if(item.billing_status === '작성 중' ){
                        //     var name = '<button id="btnModify" class="btnLink">' + item.name +'</button>';
                    ...
                    case 'status':
                        if(item.billing_status === '작성 중' || item.billing_status === '결재 대기'|| item.billing_status === '회수'){
                            var btn = '<button id="btnDel" class="btnText redTxt" type="button">삭제</button>';                                        
                        }else{
                            var btn = '<button id="btnAdd" class="btnText blueTxt" type="button" >복제</button>';  
                        }
    ...
    //삭제
    const handelDel  = useCallback(async (val) => {
        confirm({
            ...
            onOk(){
                ...
                var config = {
                    method: 'DELETE',
                    url: process.env.REACT_APP_API_URL +'/api/v1/billings/'+val,
                    headers: {
                        Accept: 'application/json',
                    },
                };
        ...
        // Modal.warning({
        //     title:"삭제하면 복구할 수 없습니다. 계속 하시겠습니까?",
        //     onOk(){
        //         var axios = require('axios');
        ...
        <Wrapper>
            ...
            <Row id="gridWrap" className="gridWrap" ref={tabRef}>
                ...
                >
                    ...
                    <FlexGridColumn binding="status" header="작업" width={100} align="center" isReadOnly={true} />
                </FlexGrid>
            ...
            <Row gutter={10} className="table table_bot">
                <Col xs={24} lg={16}>
                    <div className="btn-group">
                        <Pagination showSizeChanger={false} defaultCurrent={1} defaultPageSize={state.pageArr.pageCnt} current={state.pageArr.page} total={state.total} onChange={pageChange}/>
            ...
            {viewAddVisible === true && (
                <AddPrd
                    ...
                    onClose={addOnClose}
                    copyIdx ={state.copyId}
                    // modifyChk = {state.modifyChk }
                    reset={fetchData}
                />
            )}
