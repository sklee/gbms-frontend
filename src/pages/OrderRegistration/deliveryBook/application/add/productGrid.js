...
const ProductGrid = ( props ) =>{
    ...
    useEffect(()=>{
        if(props.id){
            ...
            const init_obj = [initRef,...props.orders]
    ...
    const columns = [
        {
            ...
            width : 100,
            align : 'right',
            render : (item,record) => isEditing(record) ? <FormikInput name={'product_qty'} readOnly={true}/> : item
        },{
            dataIndex : "price", 
            title : "정가*",
            width : 80,
            align : 'right',
            render : (item,record) => isEditing(record) ? <FormikInput name={'price'} readOnly={true}/> : item
        },{
